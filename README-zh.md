...
| 项目 | 变更 |
|------|------|
| Step 0f | 协调者经新脚本 load_uni_gotchas.sh 加载 P0/P1 级 UNI 失效模式摘要，失败优雅降级 |
| P1 D5 维度 | 新增强制「UNI 对标」审计：逐条判定适用性与结构性覆盖，适用未覆盖即产出发现 |
| Stage 1 摘要 | 维度计数更名 + 新增 `🔍 UNI 对标` 统计行；回归模式独立输出块 |
| DESIGN.md | 新增 §UNI 对标维度：机制依据与定期对标约定 |
