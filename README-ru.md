...
Квотный шлюз перед запуском для Stage 1: перед параллельным запуском 4 агентов-ревьюеров координатор выполняет `quota_report.sh --spawn 4` из quota-pilot (при отсутствии quota-pilot шаг молча пропускается). Вердикт `parallel` сохраняет текущее поведение; `serial` превращает fan-out в цепочку (P1→P2→P3→P4), чтобы предупреждения о квоте могли остановить сессию на границе юнита; `park` пишет checkpoint на чистой границе и ждёт сброса окна. Закрывает слепую зону расхода параллельных субагентов (пробел ④ quota-pilot).

### v1.2.0 (2026-07-10)

| Пункт | Изменение |
|-------|-----------|
| Step 0f | Координатор загружает сводку режимов отказа UNI P0/P1 через новый скрипт load_uni_gotchas.sh (мягкая деградация) |
| Измерение P1 D5 | Новый обязательный аудит «UNI benchmark»: применимость + структурное покрытие по каждой записи |
| Сводка Stage 1 | Новая строка статистики `🔍 UNI 对标`; отдельный блок для режима регрессии |
...
| Пункт | Изменение |
|-------|-----------|
| Контракт режима регрессии | `--regression` записывает файл-заглушку `challenger_response.md` перед запуском Reporter |
| Блокировка параллельного запуска | `init_scratch.sh` теперь использует чисто временную блокировку (`kill -0` проверял короткоживущий PID и никогда не блокировал) |
